...
fun TopBar (title: String, color: Boolean) {
    ...
    TopAppBar(
        colors = if (!color) androidx.compose.material3.TopAppBarDefaults.topAppBarColors(
            LightOrange) else androidx.compose.material3.TopAppBarDefaults.topAppBarColors(Orange),
        title = { Text(
            text = title,
            color = Color.White,
            fontSize = 32.sp,
